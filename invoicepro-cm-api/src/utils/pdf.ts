...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ NUMBER TO WORDS Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ MAIN GENERATOR Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (business?.ownerSignatureUrl) {
      try {
        ...
        } else if (!path.isAbsolute(ownerSigPath)) {
          ownerSigPath = path.join(__dirname, "../..", ownerSigPath);
        }
        
        // Skip remote URLs for now (complex to handle in sync context)
        if (!ownerSigPath.startsWith("http") && fs.existsSync(ownerSigPath)) {
    ...
    if (invoice.customerSignature) {
      try {
        ...
        } else if (!path.isAbsolute(sigPath)) {
          sigPath = path.join(__dirname, "../..", sigPath);
        }
        
        // Skip remote URLs for now
        if (!sigPath.startsWith("http") && fs.existsSync(sigPath)) {
