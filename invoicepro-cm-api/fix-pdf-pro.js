...
// 1. UNLOCK THE PDF (Remove the "must be paid" restriction)
let routesCode = fs.readFileSync('src/routes/pdf.routes.ts', 'utf8');
routesCode = routesCode.replace(
  /\/\/ 🔒 LOCK PDF UNTIL PAID[\s\S]*?}\n/,
  '// Public PDF unlocked so clients can pay via the link inside.\n'
);
fs.writeFileSync('src/routes/pdf.routes.ts', routesCode);
console.log('✅ Public PDF unlocked!');

// 2. REWRITE PDF GENERATOR WITH LOGO & PAYMENT LINK
const pdfCode = `import PDFDocument from "pdfkit";
...
console.log('✅ PDF Generator rewritten with Logo & Payment Link!');
