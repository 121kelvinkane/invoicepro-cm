...
export default function Dashboard() {
  ...
  const stats = [
    { label: t("dashboard.totalRevenue"), value: formatFCFA(revenue), icon: TrendingUp, bg: "bg-emerald-50", text: "text-emerald-600" },
    { label: t("dashboard.unpaidAmount"), value: formatFCFA(unpaidAmount), icon: CreditCard, bg: "bg-blue-50", text: "text-blue-600" },
    { label: t("dashboard.overdueInvoices"), value: overdueCount, icon: AlertTriangle, bg: "bg-red-50", text: "text-red-600" },
  ];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center justify-between hover:shadow-md transition-shadow">
            <div>
              <p className="text-sm font-medium text-gray-500">{t("dashboard.totalThisMonth")}</p>
              <p className="text-2xl font-bold text-gray-900 mt-1">{formatFCFA(revenue)}</p>
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center justify-between hover:shadow-md transition-shadow">
            <div>
              <p className="text-sm font-medium text-gray-500">{t("dashboard.unpaidAmount")}</p>
              <p className="text-2xl font-bold text-gray-900 mt-1">{formatFCFA(unpaidAmount)}</p>
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center justify-between hover:shadow-md transition-shadow">
            <div>
              <p className="text-sm font-medium text-gray-500">{t("dashboard.invoicesThisWeek")}</p>
              <p className="text-2xl font-bold text-gray-900 mt-1">{invoices.filter(i => { const d = new Date(i.createdAt); const weekAgo = new Date(); weekAgo.setDate(weekAgo.getDate() - 7); return d >= weekAgo; }).length}</p>
            </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-card mb-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-semibold text-gray-900">{t("dashboard.revenueOverview")}</h2>
            <span className="text-sm text-gray-500">{t("dashboard.last6Months")}</span>
          </div>
          <div className="h-72">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-card overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            ...
            <Link to="/invoices" className="flex items-center text-sm text-primary-600 hover:text-primary-700 font-medium">
              {t("dashboard.viewAll")} <ArrowUpRight size={14} className="ml-1" />
            </Link>
          </div>
          {loading ? (
            ...
            <div className="p-6 text-gray-500">{t("dashboard.noInvoicesYet")}</div>
          ) : (
